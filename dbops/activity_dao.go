...
func InsertActivity(activity *model.Activity) error {
	...
	_, err = stmt.Exec(sql, activity.Title, activity.Name, activity.Remark, activity.Online, activity.StartTime,
		activity.EndTime, activity.Description, activity.EntrancePicture, activity.InternalTopPicture,
		activity.Del, activity.CreateTime, activity.UpdateTime)
...
`
	...
	_, err = stmt.Exec(sql, activity.Title, activity.Name, activity.Remark, activity.Online, activity.StartTime,
		activity.EndTime, activity.Description, activity.EntrancePicture, activity.InternalTopPicture,
		0, time.Now(), time.Now(), activity.Id)
	if err != nil {
		return err
	}
	return nil

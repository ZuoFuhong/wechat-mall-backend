...
func registerHandler(router *mux.Router, cmsHandler *cms.Handler, portalHandler *portal.Handler) {
	...
	router.Handle("/api/cart/goods_num", chain.ThenFunc(portalHandler.GetCartGoodsNum)).Methods("GET")
	...
	router.Handle("/api/order/refund_apply", chain.ThenFunc(portalHandler.RefundApply)).Methods("PUT")
	router.Handle("/api/order/refund_detail", chain.ThenFunc(portalHandler.RefundDetail)).Methods("GET").Queries("refundNo", "{refundNo}")
	router.Handle("/api/order/refund_undo", chain.ThenFunc(portalHandler.UndoRefundApply)).Methods("PUT").Queries("refundNo", "{refundNo}")
	router.Handle("/api/order/remind", chain.ThenFunc(portalHandler.GetOrderRemind)).Methods("GET")
